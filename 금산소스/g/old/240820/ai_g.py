...
def perform_post(db, calib_time_post, count_post, post_chol, job_datetime):
    ...
    # ana_df_droped.to_csv('./all_preprocess.csv')
    ...
    # g_h_in_residual_cl_prep_mean_5m = round((ana_df_droped.loc[before_5m:now, 'G_H_IN_RESIDUAL_CL'].mean()).astype('float64'), 2)

    '''
    #123
    '''
    # g_h_in_residual_cl_prep_mean_5m = round((ana_df_droped.loc[before_20m:now, 'G_H_IN_RESIDUAL_CL_DEL_HUNT'].min()).astype('float64'), 2)
    # g_h_in_residual_cl_prep_mean_5m = round((g_rt_df.loc[before_20m:now, '정수지 유입 잔류염소'].min()).astype('float64'), 2)
    
    # 정수지 유입 잔류염소 최근 40분 데이터 : 역세 시 포함 정수지 유입 잔류염소 최대치가 1.3 ppm을 초과하는 지 여부 체크
    before_40m = now - timedelta(minutes=40)
    g_h_in_residual_cl_prep_mean_5m = round((g_rt_df.loc[before_40m:now, '정수지 유입 잔류염소'].min()).astype('float64'), 2)
    ...
    if ai_g_h_in_residual_cl < g_h_in_residual_cl_prep_mean_5m - 0.04:
        ai_g_h_in_residual_cl = ai_g_h_in_residual_cl + 0.02
    elif ai_g_h_in_residual_cl > g_h_in_residual_cl_prep_mean_5m + 0.04:
        ai_g_h_in_residual_cl = ai_g_h_in_residual_cl - 0.02
    ...
    # if g_h_in_obj_residual_cl - ai_g_h_in_residual_cl > g_post_chg_limit_for_onetime:
    #     ai_g_post_correct_degree = g_post_chg_limit_for_onetime
    # elif g_h_in_obj_residual_cl - ai_g_h_in_residual_cl < -g_post_chg_limit_for_onetime:
    #     ai_g_post_correct_degree = -g_post_chg_limit_for_onetime
    # else:
    #     ai_g_post_correct_degree = round(g_h_in_obj_residual_cl - ai_g_h_in_residual_cl, 2)

    if g_h_in_obj_residual_cl - g_h_in_residual_cl_prep_mean_5m > g_post_chg_limit_for_onetime:
        ai_g_post_correct_degree = g_post_chg_limit_for_onetime
    elif g_h_in_obj_residual_cl - g_h_in_residual_cl_prep_mean_5m < -g_post_chg_limit_for_onetime:
        ai_g_post_correct_degree = -g_post_chg_limit_for_onetime
    else:
        ai_g_post_correct_degree = round(g_h_in_obj_residual_cl - g_h_in_residual_cl_prep_mean_5m, 2)
        
    # 보정로직 : 후차염의 경우 목표 잔류염소와 잔류염소의 편차가 0.05 이상일 경우에만 주입률을 변경한다.
    if g_h_in_obj_residual_cl - 0.05 < g_h_in_residual_cl_prep_mean_5m < g_h_in_obj_residual_cl + 0.05:
        ai_g_post_correct_degree = 0
        
    # 최근 40분간 정수지 유입 잔류염소 max 값이 1.25 ppm 이상 1.4 ppm 이하 -> 주입률 상향 조정하려 할 시 주입률 변경하지 않음.
    if 1.25 <= g_h_in_residual_cl_40m_max <= 1.4 and ai_g_post_correct_degree > 0:
        ai_g_post_correct_degree = 0
    # 최근 40분간 정수지 유입 잔류염소 max 값이 1.3 ppm 초과 -> 주입률 하향. 그러나 평활화한 잔류염소가 목표보다 0.05 ppm 아래 수준이라 더 내리기 힘들면 주입률 변경하지 않음.
    if g_h_in_residual_cl_40m_max > 1.4:
        if g_h_in_residual_cl_prep_mean_5m < g_h_in_obj_residual_cl - 0.05:
            ai_g_post_correct_degree = 0
        else:
            ai_g_post_correct_degree = -min(0.05, g_post_chg_limit_for_onetime)
    ...
    IN_VAL_POST['G_H_IN_RESIDUAL_CL_PREP'] = g_h_in_residual_cl_prep_mean_5m
