...
def perform_pre(db, calib_time_pre, count_pre, pre_chol, job_datetime):
    ...
    # 보정로직 : 전차염의 경우 목표 혼화지 잔류염소와 혼화지 잔류염소의 편차가 0.08 이상일 경우에만 주입률을 변경한다.
    # 일반적으로 
...
def perform_post(db, calib_time_post, count_post, post_chol, job_datetime):
    ...
    if round(g_h_in_obj_residual_cl - g_h_in_residual_cl_prep, 2) >= 0.05:
        ai_g_post_correct_degree = min(g_post_chg_limit_for_onetime, round( (g_h_in_obj_residual_cl - ai_g_h_in_residual_cl - 0.005) / 3, 2) )
    elif round(g_h_in_obj_residual_cl - g_h_in_residual_cl_prep, 2) <= -0.05:
        ai_g_post_correct_degree = max(-g_post_chg_limit_for_onetime, -round( (ai_g_h_in_residual_cl - g_h_in_obj_residual_cl - 0.005) / 3, 2) ) 
    else:
