...
def perform_post(db, calib_time_post, count_post, post_chol, job_datetime):
    ...
    if round(g_h_in_obj_residual_cl - g_h_in_residual_cl_prep, 2) >= g_h_in_residual_cl_holding:
        ai_g_post_correct_degree = min(g_post_chg_limit_for_onetime, round( (g_h_in_obj_residual_cl - g_h_in_residual_cl - 0.005) / 3, 2) )
    elif round(g_h_in_obj_residual_cl - g_h_in_residual_cl_prep, 2) <= -g_h_in_residual_cl_holding:
        ai_g_post_correct_degree = max(-g_post_chg_limit_for_onetime, -round( (g_h_in_residual_cl - g_h_in_obj_residual_cl - 0.005) / 3, 2) ) 
    else:
        ai_g_post_correct_degree = round(g_h_in_obj_residual_cl - g_h_in_residual_cl, 2)   
    ...
    # 최근 40분간 정수지 유입 잔류염소 max 값이 1.5 ppm 이상 1.6 ppm 이하 -> 주입률 상향 조정하려 할 시 주입률 변경하지 않음.
