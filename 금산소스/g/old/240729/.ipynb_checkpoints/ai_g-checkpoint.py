...
def perform_pre(db, calib_time_pre, count_pre, pre_chol, job_datetime):
    ...
    ai_g_pre_consumption = round(g_pre_chol_rate - ai_g_d_residual_cl, 2)
    ...
    # 보정로직 : 전차염의 경우 목표 잔류염소와 잔류염소의 편차가 0.05 이상일 경우에만 주입률을 변경한다.
    # 일반적으로 
    ...
    FACTOR_PRE['b_in_fr'] = IN_VAL_PRE['B_IN_FR']
    FACTOR_PRE['CLI-6001'] = IN_VAL_PRE['G_D_RESIDUAL_CL']
    FACTOR_PRE['CIC-5113'] = IN_VAL_PRE['G_PRE_CHOL_RATE']
...
def perform_post(db, calib_time_post, count_post, post_chol, job_datetime):
    ...
    # 테스트 : 전처리 후 정수지 유입 잔류염소 csv 저장
    # ana_df_droped.to_csv('./g_h_residual_cl_preprocess.csv')
    ...
    # 정수지 유입 잔류염소 최근 5분 평균 데이터
    before_5m = now - timedelta(minutes=5)
    g_h_in_residual_cl_prep_mean_5m = round(ana_df_droped.loc[before_5m:now, 'G_H_IN_RESIDUAL_CL'].mean(), 2)
    ...
    # 보정로직 : 전,중차염의 경우 목표 잔류염소와 잔류염소의 편차가 0.05 이상일 경우에만 주입률을 변경한다.
    ...
    FACTOR_POST['h_ph'] = IN_VAL_POST['H_PH']
    FACTOR_POST['f_tb'] = IN_VAL_POST['G_TBI']
    FACTOR_POST['b_in_fr'] = IN_VAL_POST['B_IN_FR']
    FACTOR_POST['CLI-4011'] = IN_VAL_POST['G_H_IN_RESIDUAL_CL']
    FACTOR_POST['CLI-4021'] = IN_VAL_POST['G_H_OUT_RESIDUAL_CL']
    FACTOR_POST['CIC-5213'] = IN_VAL_POST['G_POST_CHOL_RATE']
