...
def save_ctr_tag(operation_mode, db, count, correct_degree, threshold, tb_ctr_name, tb_alm_name, UPD_TI, TAG_SN, TAG_VAL, TAG_CMP_VAL, KFK_FLG=0, CTR_FLG=0, ALM_CTR=0, ALM_THRES=0, min_chol_rate=0, max_chol_rate=2, RNTI=datetime.now().strftime('%Y-%m-%d %H:%M') ):
    UPD_TI = f'\"{RNTI}\"'
    RNTI = f'\"{RNTI}\"'
    TAG_SN = f'\"{TAG_SN}\"'
...
def perform_pre(db, calib_time_pre, count_pre, pre_chol, job_datetime):
    ...
    savgol_window_size = min(60, num_datas//2)
    ...
    ai_g_pre_consumption = g_pre_chol_rate - ai_g_d_residual_cl
    ...
    # 보정로직 : 전차염의 경우 목표 잔류염소와 잔류염소의 편차가 0.1 이상일 경우에만 주입률을 변경한다.
    # 일반적으로 
    if g_d_obj_residual_cl - 0.1 < g_d_residual_cl < g_d_obj_residual_cl + 0.1:
    ...
    # in_val_itm, out_val_itm 을 현재 로깅할 데이터만 남기도록 수정
    ...
    # 결과 테이블에 저장
    # db.save_ai_rt('TB_AI_PRE_G2_LIV_RT', df_pre_final)

    # 수동 모드일 경우에만 ctr 및 alm 제어
    # 전차염 자동/수동 설정 값이 0일 경우 수동모드임
    # is_auto_mode = g_rt_df.loc[now, '전차염 자동/수동 설정']
    # if is_auto_mode == 0:
        # 전차염 CTR 테이블에 저장
    save_ctr_tag(
...
def perform_post(db, calib_time_post, count_post, post_chol, job_datetime):
    ...
    # TODO : 데이터 상하한 범위 - 데이터 확인하고 정하기(후차염)
    # TODO : 데이터 추가하기(원수 수질 관련, 모델 입력 데이터)(후차염)
    ...
    # TODO : 스무딩 길이 수정하기(전처리했던 데이터 활용), 전차염
    num_datas = len(ana_df_droped)
    savgol_window_size = min(60, num_datas//2)
    mov_avg_window_size = min(30, num_datas//2)
    ...
    g_post_chol_rate = round(g_rt_df.loc[now, '후차염 목표주입률'].astype('float64'), 2)
    ...
    # 보정로직 : 전,중차염의 경우 목표 잔류염소와 잔류염소의 편차가 0.1 이상일 경우에만 주입률을 변경한다.
    if g_h_in_obj_residual_cl - 0.1 < g_h_in_residual_cl < g_h_in_obj_residual_cl + 0.1:
    ...
    # 수동 모드일 경우에만 ctr 및 alm 제어
    # 전차염 자동/수동 설정 값이 0일 경우 수동모드임
    # is_auto_mode = g_rt_df.loc[now, '후차염 자동/수동 설정']
    # if is_auto_mode == 0:
    # 전차염 CTR 테이블에 저장
    save_ctr_tag(
