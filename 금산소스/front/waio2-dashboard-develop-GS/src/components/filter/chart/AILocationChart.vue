...
export default {
  ...
  data() {
    ...
    ChartData: function () {
      ...
      let opacityRange = 1
      let _data = []
      if(this.$store.state.filter.latest.ai_f_location_schedule !== undefined) {
        for (let i = 1, j = 3; i < 5; i++, j--) {
          if (this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].start !== '0' 
          && this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].end !== '0') {
            _data.push({
              x: new Date(this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].start).getTime(),
              x2: new Date(this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].end).getTime(),
              y: j,
              color: colorXRange,
          ...
          if (this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].next_start !== '0' 
          && this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].next_end !== '0') {
            _data.push({
              x: new Date(this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].next_start).getTime(),
              x2: new Date(this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].next_end).getTime(),
              y: j,
              color: colorXRange,
              opacity: opacityRange,
            })
          ...
          if (this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].bw_start !== '0' 
          && this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].bw_end !== '0') {
            _data.push({
              x: new Date(this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].bw_start).getTime(),
              x2: new Date(this.$store.state.filter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1)*4)].bw_end).getTime(),
              y: j,
              color: colorBW,
            })
          }
    ...
    ChartSchedule: function () {
      ...
      if (this.$store.state.filter.latest.ai_f_location_schedule !== null) {
        ...
        if (this.selectedLine === 1) {
          chart.yAxis.categories = ['4지','3지','2지','1지']
        } else {
          chart.yAxis.categories = ['8지','7지','6지','5지']
        }
        chart.navigator = {
          enabled: false,
          series: {
