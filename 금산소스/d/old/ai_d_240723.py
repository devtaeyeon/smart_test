...
def outlier_preprocessing (d_rt_df_outlier,col,window_size,n,outlier_z_score=outlier_z_score,replace_outliers_with_mean=replace_outliers_with_mean,outlier_for_timeseries=outlier_for_timeseries) :
    ...
    if len(d_rt_df_outlier[col].unique()) == 1 : 
        ...
        return outlier_replace_n.copy()
            ...
            # 이상치 대체 Z-score 3 이상인 값들을 평균 값으로 대체 
...
def set_current_dt(current_dt):
    ...
    if current_dt != '': # 지정된 날짜가 있는 경우 해당 시점 사용
        ...
        # 지정된 날짜가 없는 경우 현재 시점에서 분단위를 절삭하여 사용
...
def d_init(db,tb_ai_init):
    ...
    # 열별 보정계수 
    if not 'd_g_step1_crt' in init_tbl['ITM'] :  
        # 자동모드시 G값 보정계수 (TMP)
        d_g_step1_crt,d_g_step2_crt= 0.35,0.35
    else : 
        d_g_step1_crt = float(init_tbl[init_tbl['ITM']=='d_g_step1_crt']['INIT_VAL'].item()) # 지별 보정게수 적용 
        d_g_step2_crt = float(init_tbl[init_tbl['ITM']=='d_g_step2_crt']['INIT_VAL'].item()) # 지별 보정게수 적용 

    # 열별 G값 상/하한
    if not 'd_g_step1_min' in init_tbl['ITM'] :  
        # G값 상/하한 (TMP)
        d_g_step1_min,d_g_step1_max = 15,16
        d_g_step2_min,d_g_step2_max = 15,16             
    else :     
...
def pre_alm_check(db,tb_ai_d_alm,alm_id):
    ...
    sql = f'''
        ...
        where ALM_TI >= CURDATE() - INTERVAL {hour} HOUR 
...
# RPM 산출식 Flag
# (1 : 1번 산출식 적용 / 2 : 2번 산출식 적용)
rpm_flag = 2

# G 값 Flag 
# (1: 태그 데이터 G값 사용 / 2: init 테이블 내 수동 G값 사용/ 3: 운영 표준화 식 G값 사용 / 9: (준공도서) 고정된 G값 사용)
g_flag = 2

# Hz 제어 Flag
# (0 : Hz 단위 미사용 / 1 : Hz 단위 사용)
hz_flag = 1
...
# fri_col_2 = '771-358-FRI-7612'# tmp
# tb_e_col = '771-358-TBI-6002' # tmp 
...
# 알람 ID  
alm_id1 = 133005     # (반자동모드) 혼화응집 기존 Hz 대비 일정범위 이상
alm_id2 = 133004     # (자동모드) 혼화응집 Hz 변경
alm_id3 = 133002     # 혼화응집 AI 분석 이상

# ! -- common 내 RPM_FORMULA도 같이 업데이트해서 가져가야 함 -- ! 
# 모델 실행함수
@log_perform
def perform(db, job_datetime=None):
    try : 
        ...
        # 전처리 후에도 결측이 발생하는 경우 결측치 처리 
        # 데이터 수집 이상등으로 건수가 충분하지 않아 전처리가 제대로 되지 않은 경우,
        # 입력변수 기존 5개년 평균치로 대체 
        for col in tags:
            if col in dic_mean : 
        ...
        # G값 자동모드
            ...
            # 자동모드 보정계수 적용
            # 금산의 경우, 단별로 G값 차이가 없으므로 단별 증/감율 적용하지 않음 
        ...
        # 응집기 RPM 현재 값
        d_fc_location_sp_json_str = dict() # 최종 dict
        to_dict_location = dict() # location, step, 열 정보에 따른 값이 저장될 dict
        for pond in list_pond_nm:
            to_dict_step  = dict() # step, 열 정보에 따른 값이 저장될 dict
            for i in range(2):
        ...
        # 예측 응집기 RPM 설정값 
        ai_d_fc_location_sp2_json_str = dict() # 최종 dict
        to_dict_location = dict() # location, step, 열 정보에 따른 값이 저장될 dict
        for j, pond in enumerate(list_pond_nm): 
            # step, 열 정보에 따른 값이 저장될 dict
            to_dict_step  = dict()
        ...
        # 응집기 ON 상태 
        d_fc_location_state_json_str = dict() # 최종 dict
        to_dict_location = dict() # location, step, 열 정보에 따른 값이 저장될 dict
        for pond in list_pond_nm:
            to_dict_step  = dict() # step, 열 정보에 따른 값이 저장될 dict
            for i in range(2):
        ...
        #### CTR TBL - INSERT ###
        alm_cnt_1 = [] # 133005 : 혼화응집 기존 Hz 대비 일정범위 이상
        alm_cnt_2 = 0  # 133004 : 혼화응집 Hz 변경 

        # 알람 팝업 주기 
        pre_alm_yn_1 = 0 # 133005 : 혼화응집 기존 Hz 대비 일정범위 이상
        pre_alm_yn_2 = 0 # 133004 : 혼화응집 Hz 변경 
    
        # 알람 팝업 주기 (현재 미적용) 
        # pre_alm_yn_1 = pre_alm_check(db,tb_ai_d_alm,alm_id1) # 133005 : 혼화응집 기존 Hz 대비 일정범위 이상
        # pre_alm_yn_2 = pre_alm_check(db,tb_ai_d_alm,alm_id2) # 133004 : 혼화응집 Hz 변경 
        ...
        if d_operation_mode != 0  : 
            ...
            # 지/열별 알람 임계치 조건
            # 예측 값 대비 현재 값의 비율이 10% 이상 차이나는 경우 알람 
            # 원수 수온 기준 10도 이상 차이는 경우와 동일 
            d_alm_thrshld_undr = lambda dict_sp, loc_no, cmp_val: round(round(dict_sp['location{}'.format(loc_no)],0)/cmp_val, 2) <= 0.90
            d_alm_thrshld_over = lambda dict_sp, loc_no, cmp_val: round(round(dict_sp['location{}'.format(loc_no)],0)/cmp_val, 2) >= 1.10
            ...
            for loc_id, loc_val in dict_sp.items():
                ...
                # 자동모드 알람/제어 
                    ...
                    # G값 수동모드 해당 
                    if d_g_value_ctr_flag == 0 :
                        # 한 건이라도 임계치를 초과하는 경우, 모든 열을 임계치 팝업으로 제어  
                        if alm_thrshld_yn == 1 : 
                            # 최근 12시간 이력 없는 경우
                            if pre_alm_yn_1 == 0 : 
                                # 열별로 알람이 전송된적이 없는 경우 최초 1회만 전송
                        ...
                        # 임계치 기준을 초과하지 않는 경우 자동모드 제어 
                            ...
                            if pre_alm_yn_2 == 0 : 
                                # 1회 모델 실행 시 알람이 전송된적이 없는 경우 최초 1회만 전송
                                if alm_cnt_2 == 0  : 
                                    send_alm(db=db,table=tb_ai_d_alm,alm_id=alm_id2,alm_ti=str(pd.to_datetime(df_mixing_ai_result['upd_ti'][0])))
                                    alm_cnt_2 += 1
                                # 제어 결과 적재
        ...
        send_alm(db=db,table=tb_ai_d_alm,alm_id=alm_id3,alm_ti=time.strftime('%Y-%m-%d %H:%M:%S'))
        print('err occured')
