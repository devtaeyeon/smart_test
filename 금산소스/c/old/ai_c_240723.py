...
def perform(db, job_datetime=None):
    ...
    ai_opr, co_max, co_min = get_init(db) # INIT 가져오기
    ...
    # 탁도 5NTU미만 시 10ppm / 5~20NTU일 때 12~14ppm / 20NTU 이상 시 15ppm 주입
    ...
    if ai_opr == 1:
        ...
        if abs(ai_pred_y_set - last_set) >= 1:
            # 임계치 범위를 벗어나는 경우 4번 타입 알람 발생
            if (ai_pred_y_set > co_max) or (ai_pred_y_set < co_min):
            ...
            if set_df['run1'].loc[current_dt_date] == 1: # 1호기 가동 시
                ...
                ctr_set(db, run_time, control_tag, ai_pred_y_set, last_set)
    ...
    if ai_opr == 2:
        ...
        if abs(ai_pred_y_set - last_set) >= 1:
            ...
            if (ai_pred_y_set <= co_max) and (ai_pred_y_set >= co_min):
                db.save_alm('TB_AI_C_ALM', 132004, run_time)
                print('---------------- 응집제 주입률 변경 알람 발생 ----------------')
            # 임계치 범위를 벗어나는 경우 4번 타입 알람 발생
            elif (ai_pred_y_set > co_max) or (ai_pred_y_set < co_min):
                db.save_alm('TB_AI_C_ALM', 132006, run_time)
                print('---------------- 응집제 주입률 임계치 알람 발생 ----------------')
            # 응집제 주입률 제어
            if set_df['run1'].loc[current_dt_date] == 1: # 1호기 가동 시
