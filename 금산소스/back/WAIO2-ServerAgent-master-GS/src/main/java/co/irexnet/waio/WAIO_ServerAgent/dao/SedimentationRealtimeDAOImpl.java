...
public class SedimentationRealtimeDAOImpl implements IProcessRealtimeDAO
{
    ...
    public int insert(List<ProcessRealtimeDTO> list)
    {
        int[] result = jdbcTemplate.batchUpdate(
                "INSERT IGNORE INTO TB_E2_RT VALUES(?, ?, ?, ?);",
    ...
    public void addPartition(String partitionName, String end_time)
    {
        for(int processStep : CommonValue.PROCESS_STEP_ARRAY) {
            String strQuery = String.format("ALTER TABLE TB_E" + processStep + "_RT REORGANIZE PARTITION p_max INTO (" +
    ...
    {
        for(int processStep : CommonValue.PROCESS_STEP_ARRAY) {
            String strQuery = String.format("ALTER TABLE TB_E" + processStep + "_RT DROP PARTITION p_%s", partitionName);
            jdbcTemplate.execute(strQuery);
        }
    }
