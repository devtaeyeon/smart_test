...
public class KafkaConsumer
{
    ...
    public void listenTagInfo(List<ConsumerRecord<String, String>> records)
    {
    	...
    	//260331 이현수 IRDB적용(청주,천안,석성,금산) 요청자: 조율산과장 
        ...
        //nSendToBDCount 260331 이현수 IRDB적용(청주,천안,석성,금산) 요청자: 조율산과장 
    ...
    public void listenScada(List<ConsumerRecord<String, String>> records)
    {
        ...
        // scd1ToBdSuccessCnt,scd2ToBdSuccessCnt 260331 이현수 IRDB적용(청주,천안,석성,금산) 요청자: 조율산과장
        ...
        for(ProcessRealtimeDTO realtime: kafkaRealtimeValue) {
            for(TagManageDTO tagManage: tagManageList) {
                ...
                if(tagManage.getTag_ty() == CommonValue.TAG_MANAGE_TYPE_INIT) {
                    if(tagManage.getAi_cd().equalsIgnoreCase(CommonValue.PROCESS_COAGULANT) == true && CommonValue.C_OPERATION_MODE.equalsIgnoreCase(tagManage.getItm()) == true) {
                        if(tagManage.getTag_sn().equalsIgnoreCase(realtime.getTag_sn()) == true &&
                            ...
                            nCoagulantUpdateCount +=
                                    databaseService.modAiCoagulantInit(tagManage.getItm(), Float.parseFloat(realtime.getTag_val()), 1);
                        }
                        ...
                        if(tagManage.getTag_sn().equalsIgnoreCase(realtime.getTag_sn()) == true &&
                                Conversion.isNumber(realtime.getTag_val()) == true) {
                            nMixingUpdateCount +=
                                    databaseService.modAiMixingInit(tagManage.getItm(), Float.parseFloat(realtime.getTag_val()), 1);
                        }
                        ...
                        if(tagManage.getTag_sn().equalsIgnoreCase(realtime.getTag_sn()) == true &&
                    			...
                    			if(CommonValue.G_PRE_OPERATION_MODE.equalsIgnoreCase(tagManage.getItm())== true) {
                    				nDisinfectionUpdateCount +=
                    					databaseService.modAiDisinfectionInit(tagManage.getItm(), Float.parseFloat(realtime.getTag_val()), 1, CommonValue.DISINFECTION_PRE_STEP); // 전차염
                    			}else if(CommonValue.G_POST_OPERATION_MODE.equalsIgnoreCase(tagManage.getItm())== true) {
                    				nDisinfectionUpdateCount +=
                    						databaseService.modAiDisinfectionInit(tagManage.getItm(), Float.parseFloat(realtime.getTag_val()), 1, CommonValue.DISINFECTION_POST_STEP); // 후차염                            	                                                    				
                    			}
                            }
                        }
                    continue;
        ...
        for(String procCd: procCdList)
        {
            ...
            if(processRealtime != null)
            {
                try{
                    ...
                    // log.debug("[Collector]Success load to table {}: {}", procCd, databaseService.addProcessRealtimeValue(procCd, processRealtime));
