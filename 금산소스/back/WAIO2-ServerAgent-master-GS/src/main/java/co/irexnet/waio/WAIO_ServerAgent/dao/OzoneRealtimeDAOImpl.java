...
public class OzoneRealtimeDAOImpl implements IProcessRealtimeDAO
{
    ...
    public int insert(List<ProcessRealtimeDTO> list)
    {
        int[] result = jdbcTemplate.batchUpdate(
                "INSERT IGNORE INTO TB_IO_RT VALUES(?, ?, ?, ?);",
    ...
    {
        String strQuery = String.format("SELECT * FROM TB_IO_RT PARTITION(p_%s) WHERE upd_ti IN " +
                "(SELECT MAX(upd_ti) FROM TB_IO_RT PARTITION(p_%s) GROUP BY tag_sn)", partitionName, partitionName);
        ...
        {
            strQuery = String.format("SELECT * FROM TB_IO_RT PARTITION(p_max) WHERE upd_ti IN " +
                    "(SELECT MAX(upd_ti) FROM TB_IO_RT PARTITION(p_max) GROUP BY tag_sn)");
    ...
    {
        String strQuery = "SELECT * FROM TB_IO_RT WHERE upd_ti > ? ORDER BY upd_ti DESC";
    ...
    {
        String strQuery = "SELECT * FROM TB_IO_RT WHERE tag_sn=? AND upd_ti > ? AND upd_ti < ?";
        return jdbcTemplate.query(
                strQuery,
    ...
    {
        String strQuery =  "SELECT * FROM TB_IO_RT WHERE tag_sn=? ORDER BY upd_ti DESC limit 1";
        try
        {
    ...
    {
        String strQuery = String.format("ALTER TABLE TB_IO_RT REORGANIZE PARTITION p_max INTO (" +
    ...
    {
        String strQuery = String.format("ALTER TABLE TB_IO_RT DROP PARTITION p_%s", partitionName);
        jdbcTemplate.execute(strQuery);
    }
}
