...
public class AiGacControlDAOImpl implements IAiProcessControlDAO
{
    ...
    {
        String strQuery = "SELECT * FROM TB_AI_I_CTR WHERE upd_ti=? AND rnti=? AND tag_sn=? AND kfk_flg=?";
    ...
    public List<AiProcessControlDTO> select(Date rnti, int kfk_flg, int processStep)
    {
        String strQuery = "SELECT * FROM " +
                "(SELECT * FROM TB_AI_I_CTR WHERE rnti>=? AND kfk_flg=? ORDER BY rnti DESC LIMIT 100) " +
                "TB_AI_I_CTR GROUP BY tag_sn";
    ...
    {
        String strQuery = "UPDATE TB_AI_I_CTR SET kfk_flg=? WHERE upd_ti=? AND rnti=? AND tag_sn=?";
        return jdbcTemplate.update(strQuery, kfk_flg, upd_ti, rnti, tag_sn);
    }

    @Override
    public int delete(Date date, int processStep)
    {
        String strQuery = "DELETE FROM TB_AI_I_CTR WHERE upd_ti < ?";
