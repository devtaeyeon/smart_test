...
public class AiGacAlarmDAOImpl implements IAiProcessAlarmDAO
{
    ...
    {
        String strQuery = "insert into TB_AI_I_ALM values (?, ?, ?)";
    ...
    {
        String strQuery = "SELECT * FROM TB_AI_I_ALM WHERE alm_ti>=? AND kfk_flg=? ORDER BY alm_ti DESC";
    ...
    {
        String strQuery = "UPDATE TB_AI_I_ALM SET kfk_flg=? WHERE alm_id=? AND alm_ti=?";
    ...
    {
        String strQuery = "DELETE FROM TB_AI_I_ALM WHERE alm_ti<?";
