...
//                + ", JSON_UNQUOTE(JSON_EXTRACT(OUT_VAL, '$.AI_C_CF')) AS ai_c_cf"
//                + ", JSON_UNQUOTE(JSON_EXTRACT(OUT_VAL, '$.C_TB')) AS c_tb FROM " 
				+ ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AI_C_CF'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AI_C_CF'))-2) AS ai_c_cf"
                + ", SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].C_TB_E'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].C_TB_E'))-2) AS c_tb"
                + " FROM "
...
//              + ", JSON_UNQUOTE(JSON_EXTRACT(OUT_VAL, '$.C_TB')) AS c_tb FROM "  
        		...
        		+ ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].C_TB'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].C_TB'))-2) AS c_tb"
