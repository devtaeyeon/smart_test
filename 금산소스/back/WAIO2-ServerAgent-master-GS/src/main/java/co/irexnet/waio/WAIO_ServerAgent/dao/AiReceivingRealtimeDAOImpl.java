...
public class AiReceivingRealtimeDAOImpl implements IAiReceivingRealtimeDAO
{
    ...
    public AiReceivingRealtimeDTO select(int processStep)
    {
        ...
        + ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AI_B_PUMP_CB'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AI_B_PUMP_CB'))-2) AS ai_b_pump_cb "                /* 펌프 가동 조합 예측 */
        ...
        + " FROM "
        + getTableByProcessStep(processStep) + " ORDER BY upd_ti ASC LIMIT 1"; // FIXME DESC수정
		try
        {
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) {
            tableNm = "TB_AI_B_LIV_RT";
        } else if(processStep == 2) {
            tableNm = "TB_AI_B_IND_RT";
        }
        return tableNm;
    }
}
