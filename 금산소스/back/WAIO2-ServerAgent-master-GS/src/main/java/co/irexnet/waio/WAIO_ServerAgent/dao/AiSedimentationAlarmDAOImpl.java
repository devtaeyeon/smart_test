...
public class AiSedimentationAlarmDAOImpl implements IAiProcessAlarmDAO
{
    ...
    {
        String strQuery = "SELECT * FROM "+getTableByProcessStep(processStep)+" WHERE alm_ti>=? AND kfk_flg=? ORDER BY alm_ti DESC";
    ...
    public String getTableByProcessStep(int processStep) {
    	String tableNm = "";
    	if(processStep == 1) {
    		tableNm = "TB_AI_E_LIV_ALM";
    	}else if(processStep == 2) {
    		tableNm = "TB_AI_E_IND_ALM";
    	}
    	return tableNm;
    }
