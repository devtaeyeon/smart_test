...
public class AiSedimentationRealtimeDAOImpl implements IAiSedimentationRealtimeDAO
{
    ...
    public AiSedimentationRealtimeDTO select(int processStep)
    {
        ...
        + ", CONCAT('{\"AIE-9004\":', SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AIE-9004'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AIE-9004'))-2),'}') AS AIE_9004 FROM " 
//        + getTableByProcessStep(processStep) + " ORDER BY upd_ti DESC LIMIT 1";
        + getTableByProcessStep(processStep) + " ORDER BY upd_ti ASC LIMIT 1";//TODO 운영반영시 DESC변경 필요
		try
        {
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) {
            tableNm = "TB_AI_E_LIV_RT";
        } else if(processStep == 2) {
            tableNm = "TB_AI_E_IND_RT";
        }
        return tableNm;
    }
}
