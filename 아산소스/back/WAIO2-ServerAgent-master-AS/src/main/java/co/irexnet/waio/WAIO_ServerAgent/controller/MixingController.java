...
public class MixingController {
    ...
    public ResponseEntity<String> getFcAgHistoryMixing(@RequestBody InterfaceDateSearchDTO dateSearchDTO,
        ...
        log.debug("getSpFcHistoryMixing, start:[{}], end:[{}]", dateSearchDTO.getStart_time(),
        	...
        	if(aiMixingInit != null) {
        		...
        		for (TagManageDTO dto : tagManageList) {
        			if (dto.getItm().equalsIgnoreCase("d_operation_mode_a") == true
        					&& CommonValue.PROCESS_MIXING.concat(String.valueOf(processStep)).equals(dto.getProc_cd())) {
        				// Kafka에 전송할 값 정의
    ...
    public ResponseEntity<String> putAiControlMixing(@RequestBody InterfaceMixingAiDTO mixingAi,
        ...
        if (mixingAi.isJiModifyMode()) { // 지별 상하한
            result = (databaseService.modAiMixingInit("d_g_step1_min", mixingAi.getD_g_step1_min(), processStep) == 1)
                    && result;
            result = (databaseService.modAiMixingInit("d_g_step1_max", mixingAi.getD_g_step1_max(), processStep) == 1)
                    && result;
            result = (databaseService.modAiMixingInit("d_g_step2_min", mixingAi.getD_g_step2_min(), processStep) == 1)
                    && result;
            result = (databaseService.modAiMixingInit("d_g_step2_max", mixingAi.getD_g_step2_max(), processStep) == 1)
                    && result;
            result = (databaseService.modAiMixingInit("d_g_step3_min", mixingAi.getD_g_step3_min(), processStep) == 1)
                    && result;
            result = (databaseService.modAiMixingInit("d_g_step3_max", mixingAi.getD_g_step3_max(), processStep) == 1)
                    && result;
        } else if (mixingAi.isAutoModeCrt()) { // 자동모드상세
            result = (databaseService.modAiMixingInit("d_g_step1_crt", mixingAi.getD_g_step1_crt(), processStep) == 1)
                    && result;
            result = (databaseService.modAiMixingInit("d_g_step2_crt", mixingAi.getD_g_step2_crt(), processStep) == 1)
                    && result;
            result = (databaseService.modAiMixingInit("d_g_step3_crt", mixingAi.getD_g_step3_crt(), processStep) == 1)
                    && result;
        } else {
            if (mixingAi.getD_g_value_ctr_flag() == 0) { // 수동모드
                // update 응집기 1단 교반강도 G 값
                result = (databaseService.modAiMixingInit("d_g_value_step1", mixingAi.getD_g_value_step1(),
                        processStep) == 1) && result;
                // update 응집기 2단 교반강도 G 값
                result = (databaseService.modAiMixingInit("d_g_value_step2", mixingAi.getD_g_value_step2(),
                        processStep) == 1) && result;
                // update 응집기 3단 교반강도 G 값
                result = (databaseService.modAiMixingInit("d_g_value_step3", mixingAi.getD_g_value_step3(),
                        processStep) == 1) && result;
            }
            // update G값 제어 여부
            result = (databaseService.modAiMixingInit("d_g_value_ctr_flag", mixingAi.getD_g_value_ctr_flag(),
                    processStep) == 1) && result;
        }
