...
public class AiPreDisinfectionControlDAOImpl implements IAiProcessControlDAO
{
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) { // 1단계공업 전차염
            tableNm = "TB_AI_PRE_G1_CTR";
        } else if(processStep == 2) { // 2단계생활 전차염
            tableNm = "TB_AI_PRE_G2_CTR";
        } else if(processStep == 3) { // 3단계공업 전차염
            tableNm = "TB_AI_PRE_G3_CTR";
        }
        return tableNm;
    }
