...
public class AiCoagulantInitDAOImpl implements IAiProcessInitDAO
{
    ...
    {
        String strQuery = "UPDATE " + getTableByProcessStep(processStep) + " SET INIT_VAL=? WHERE itm='c_operation_mode'"; // TODO 추후에 수정
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) {
            tableNm = "TB_AI_C1_INIT";
        } else if(processStep == 2) {
            tableNm = "TB_AI_C2_INIT";
        } else if(processStep == 3) {
            tableNm = "TB_AI_C3_INIT";
        }
        return tableNm;
    }
}
