...
public class AiMixingInitDAOImpl implements IAiProcessInitDAO
{
    ...
    {
        String strQuery = "UPDATE " + getTableByProcessStep(processStep) + " SET init_val=? WHERE itm='d_operation_mode'";
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) {
            tableNm = "TB_AI_D1_INIT";
        } else if(processStep == 2) {
            tableNm = "TB_AI_D2_INIT";
        } else if(processStep == 3) {
            tableNm = "TB_AI_D3_INIT";
        }
        return tableNm;
    }
}
