...
public class AiFilterInitDAOImpl implements IAiProcessInitDAO
{
    ...
    {
        String strQuery = "SELECT * FROM TB_AI_F2_INIT";
    ...
    {
        String strQuery = "SELECT * FROM TB_AI_F2_INIT WHERE itm=?";
        try
        {
    ...
    {
        String strQuery = "UPDATE TB_AI_F2_INIT SET init_val=? WHERE itm='f_operation_mode'";
    ...
    {
        String strQuery = "UPDATE TB_AI_F2_INIT SET init_val=? WHERE itm=?";
        return jdbcTemplate.update(strQuery, init_val, itm);
    }

    public int updateTi(float init_val, String itm)
    {
        String strQuery = "UPDATE TB_AI_F2_INIT SET init_val = ? WHERE itm=?";
        return jdbcTemplate.update(strQuery, init_val, itm);
    }
