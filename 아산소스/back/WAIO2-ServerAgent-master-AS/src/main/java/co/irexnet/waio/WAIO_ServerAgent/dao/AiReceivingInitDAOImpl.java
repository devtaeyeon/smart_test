...
public class AiReceivingInitDAOImpl implements IAiProcessInitDAO
{
    ...
    {
        String strQuery = "UPDATE " + getTableByProcessStep(processStep) + " SET init_val=? WHERE itm='b_operation_mode'";
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) {
            tableNm = "TB_AI_B1_INIT";
        } else if(processStep == 2) {
            tableNm = "TB_AI_B2_INIT";
        } else if(processStep == 3) {
            tableNm = "TB_AI_B3_INIT";
        }
        return tableNm;
    }
}
