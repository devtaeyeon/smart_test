...
public class FilterRealtimeDAOImpl implements IProcessRealtimeDAO
{
    ...
    public int insert(List<ProcessRealtimeDTO> list)
    {
        int[] result = jdbcTemplate.batchUpdate(
                "INSERT IGNORE INTO TB_F2_RT VALUES(?, ?, ?, ?);",
    ...
    public List<ProcessRealtimeDTO> select(String partitionName, int processStep)
    {
        String strQuery = String.format("SELECT * FROM " + getTableByProcessStep(processStep) + " PARTITION(p_%s) WHERE upd_ti IN " +
                "(SELECT MAX(upd_ti) FROM " + getTableByProcessStep(processStep) + " PARTITION(p_%s) GROUP BY tag_sn)", partitionName, partitionName);
    ...
    public void addPartition(String partitionName, String end_time)
    {
        for (int processStep : CommonValue.PROCESS_STEP_ARRAY) {
            String strQuery = String.format("ALTER TABLE TB_F" + processStep + "_RT REORGANIZE PARTITION p_max INTO (" +
    ...
    {
        for (int processStep : CommonValue.PROCESS_STEP_ARRAY) {
            String strQuery = String.format("ALTER TABLE TB_F" + processStep + "_RT DROP PARTITION p_%s", partitionName);
            jdbcTemplate.execute(strQuery);
        }
    }
